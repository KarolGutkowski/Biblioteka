...
<body>
    ...
    <main>
        <h2 class='titles'>wyszukiwanie</h2>
        <form method='GET'>
            <input type="text" name="search" class="inputs" placeholder="wpisz wyszukiwany tytuł">
            <p></p>
            <button type="submit" class="orange_button">szukaj</button>
        </form>
    </main>
    ...
    <?php
        if( isset($_GET['search']) ) {
            ...
            if($db) {
                $sql ="SELECT * FROM ksiazka WHERE tytul LIKE '$search%' LIMIT 1";
                $query = mysqli_query($db, $sql);
                if( mysqli_num_rows($query) > 0) {
                    $results = mysqli_fetch_array($query);
                    $photo = $results['zdjecie'];
                    echo "<img src='$photo' alt='zdjecie wyszukiwanej ksiazki' class='searched-image'>";
                    echo '<p>Tytuł: ' .$results['tytul'] .'</p>';
                    echo '<p>Autor: '.$results['autor'] .'</p>';
                    echo '<p>Rok Wydania: ' .$results['rok_wydania'] .'</p>';
                    echo '<p>Gatunek: ' .$results['gatunek'] .'</p>';
                    echo '<p>Dostępność: ' .$results['dostępna'] .'</p>';
                    echo "<p class='ocena'>" .$results['ocena'] .'/5 </p>';
                    echo "<p class='rating' > Ocena czytelników </p>";
                } else {
                    echo 'Niestety nie możemy znaleźć wpisanego przez ciebie tytyłu :(';
                }
            }
        }
    ...
    <script src='./js/search-dark_mode.js'></script>
</body>
</html>